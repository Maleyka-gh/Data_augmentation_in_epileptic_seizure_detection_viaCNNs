...
Therefore, two datasets are created from the original data: one with five features (3D Acceleration in x,y,z direction, heart rate and temperature) and the other with only three features (only 3D Acceleration).
...
to the original samples, such as adding random noise (jittering) or applying warping
in time and magnitude. Other techniques include rearranging segments(permutation)
within the time series, applying rotation matrices(rotation), or slicing (window slicing). 
