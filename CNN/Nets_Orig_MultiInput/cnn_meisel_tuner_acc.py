...
class CNN_Hyper:
    def __init__(self, result, x_train, y_train, x_valid, y_valid, times_valid, path, k, cw=None, name=""):
        ...
        print(name)
        info = self.set_info()
        self.prep = preparator.Preparator(result, x_train, y_train, x_valid, y_valid, times_valid, path, k, cw=cw,
                                          name=name, info=info)
        ...
        train_feat = [self.prep.train_x_acc_x, self.prep.train_x_acc_y, self.prep.train_x_acc_z]
        epochs = 2000
        batch_size = 32
        ...
        valid_feat = [self.prep.valid_x_acc_x, self.prep.valid_x_acc_y, self.prep.valid_x_acc_z]
    ...
    def create_model(self, input_shape):
        ...
        concatenated = concatenate([acc_x_model.output, acc_y_model.output, acc_z_model.output])

        out = Dense(self.prep.num_classes, activation='softmax', name='output_layer')(concatenated)

        model = Model([acc_x_model.input, acc_y_model.input, acc_z_model.input], out)
