...
# from tensorflow.python.layers.layers import dense,flatten
# from tensorflow.python.keras.initializers.initializers_v1 import _v1_glorot_uniform_initializer
...
def downconv(x, output_dim, kwidth=5, pool=2, init=None, uniform=False,
    ...
    w_init = init
    if w_init is None:
        # w_init = _v1_glorot_uniform_initializer()
        w_init = tf.compat.v1.keras.initializers.GlorotUniform()
...
def atrous_conv1d(value, dilation, kwidth=3, num_kernels=1,
    ...
    # print('Input_channel',input_shape[-1])
    in_channels = input_shape[-1]
    assert len(input_shape) >= 3
    with tf.compat.v1.variable_scope(name):
