...
class GAN(Model):
    ...
    def __init__(self, sess, args, devices, infer=False, name='GAN'):
        ...
        self.g_dec_depths = self.g_enc_depths[:-1][::-1] + [3]
        # Define D fmaps
    ...
    def build_model(self, config):
        ...
        d_opt = tf.compat.v1.train.AdamOptimizer(config.d_learning_rate, beta1=config.beta_1, beta2=config.beta_2)
        g_opt = tf.compat.v1.train.AdamOptimizer(config.g_learning_rate, beta1=config.beta_1, beta2=config.beta_2)
    ...
    def build_model_single_gpu(self, gpu_idx):
        ...
        if gpu_idx == 0:
            ...
            print('HEREE',self.get_seiz)
        # load the data to input pipeline

        seiz_batch, nonseiz_batch = tf.compat.v1.train.shuffle_batch([self.get_seiz, self.get_nonseiz],
        ...
        if gpu_idx == 0:
            ...
            dummy_joint = tf.concat([seiz_batch, nonseiz_batch], 2)
        ...
        D_rl_joint = tf.concat([seiz_batch, nonseiz_batch], 2)
        D_fk_joint = tf.concat([G, nonseiz_batch], 2)
        # build discriminator
        ...
        ### Discriminator loss ###
        d_rl_loss = tf.reduce_mean(tf.math.squared_difference(d_rl_logits, 1.))
        d_fk_loss = tf.reduce_mean(tf.math.squared_difference(d_fk_logits, 0.))
        g_adv_loss = tf.reduce_mean(tf.math.squared_difference(d_fk_logits, 1.))
        d_loss = d_rl_loss + d_fk_loss
        ### Genrator loss ###
        g_l1_loss = self.l1_lambda * tf.reduce_mean(tf.abs(tf.math.subtract(G, seiz_batch)))
    ...
    def train(self, config, devices):
        ...
        print("Heloo",self.gtruth_nonseiz[0].shape)
        sample_nonseiz, sample_seiz, sample_z = self.sess.run([self.gtruth_nonseiz[0], self.gtruth_seiz[0], self.zs[0]])
        tf.compat.v1.disable_eager_execution()
        # print(sample_z)
        ...
        num_examples = 0
        print('outt')
        for record in tf.compat.v1.io.tf_record_iterator(self.e2e_dataset):
            num_examples += 1
    ...
    def transform(self, x):
        ...
        for beg_i in range(0, x.shape[0], self.canvas_size):
            ...
            #     pad = 0
            x_ = np.zeros((self.batch_size, self.canvas_size,3))
            # if pad > 0:
            ...
            print(canvas_w.shape)
            # canvas_w = canvas_w.reshape(self.canvas_size,3)
