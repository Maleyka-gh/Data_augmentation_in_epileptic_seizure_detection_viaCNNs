...
flags.DEFINE_integer("canvas_size", 512,
                     "Canvas size (Def: 512).")  # flags.DEFINE_integer("canvas_size", 2**14, "Canvas size (Def: 2^14).")
...
flags.DEFINE_string("weights", None, "Weightsh file")
FLAGS = flags.FLAGS
...
def main(_):
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        ### TRANSFORM NON-SEIZURES INTO SEIZURES ###
            ...
            nonseiz_files = [os.path.join(FLAGS.test_set, sample) for sample in os.listdir(FLAGS.test_set) if
                             sample.endswith('.mat')]
            for m, sample_file in enumerate(nonseiz_files):
                ...
                print("SampleDat", sample_data.shape)
                # sample_data = np.transpose(sample_data, (1,0))
                ...
                print('Done transforming {} and saved to {}'.format(sample_file,
                                                                    os.path.join(FLAGS.save_transformed_path,
                                                                                 samplename)))
