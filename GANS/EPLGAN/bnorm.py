...
class VBN(object):
    ...
    def __init__(self, x, name, epsilon=1e-5):
        ...
        with tf.compat.v1.variable_scope(name) as scope:
            ...
            self.name = name
            self.mean = tf.reduce_mean(x, [0, 1], keepdims=True)
            self.mean_sq = tf.reduce_mean(tf.square(x), [0, 1], keepdims=True)
    ...
    def __call__(self, x):
        ...
        with tf.compat.v1.variable_scope(self.name) as scope:
            ...
            new_mean = tf.reduce_mean(x, [0, 1], keepdims=True)
            new_mean_sq = tf.reduce_mean(tf.square(x), [0, 1], keepdims=True)
