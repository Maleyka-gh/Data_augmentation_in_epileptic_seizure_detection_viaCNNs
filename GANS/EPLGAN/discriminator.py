...
def discriminator(self, wave_in, reuse=False):
    ...
    with tf.compat.v1.variable_scope('d_model') as scope:
        if reuse:
            scope.reuse_variables()
        else:
            print('*** Discriminator summary ***')
        for block_idx, fmaps in enumerate(self.d_num_fmaps):
            with tf.compat.v1.variable_scope('d_block_{}'.format(block_idx)):
                if not reuse:
                    print('D block {} input shape: {}'.format(block_idx, hi.get_shape()))
                downconv_init = tf.compat.v1.truncated_normal_initializer(stddev=0.02)
        ...
        d_logit_out = tf.compat.v1.layers.dense(d_logit_out, 1, activation=None)
        if not reuse:
