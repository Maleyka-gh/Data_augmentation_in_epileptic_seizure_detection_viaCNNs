...
# Global Path
LearnDf_path = r"/data/WORK/dataframes_learn_all_feat"
...
def is_num_values_ok(real_num, target_num, k=0.15):
    """
...
def do_window(path, Elements, class_labels, window_settings):
    ...
    while True:  # pythonic do-while
        ...
        next_step = normal_step / 2  # for computation of the next section/ 50 percent overlap
...
w = Windowing(window_length_sec=10, label_additional_time=5)

# In[9]:


# compute_windows(augDpath_Rotation,"Rotation",w)
compute_windows(r'/data/Data_prep_for_gan/GAN_seiz_data', "TTS_GAN_Seiz_numpy_acc_only", w)
