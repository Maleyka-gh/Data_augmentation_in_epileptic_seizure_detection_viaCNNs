...
class Running_Or_Jumping(Dataset):
    def __init__(self,
                 ...
                 is_normalize=False,
                 split_subj=dict
                     (train_subj=[4, 5, 6, 7, 8, 10, 11, 12, 14, 15, 19, 20, 21, 22, 24, 26, 27, 29, 1, 9, 16, 23, 25,
                                  28],
        ...
        if self.data_mode == 'Train':
            ...
            print(
                f'Jumping label is 1, has {len(self.Jumping_train_labels)} samples, Running label is 0, has {len(self.Running_train_labels)} samples')
        else:
            print(f'data shape is {self.combined_test_data.shape}, label shape is {self.combined_test_label.shape}')
            print(
                f'Jumping label is 1, has {len(self.Jumping_test_labels)} samples, Running label is 0, has {len(self.Running_test_labels)} samples')
