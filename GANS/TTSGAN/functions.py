...
def train_d(args, gen_net: nn.Module, dis_net: nn.Module, dis_optimizer, train_loader, epoch, writer_dict, fixed_z,
            schedulers=None):
    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
        ...
        z = torch.cuda.FloatTensor(np.random.normal(0, 1, (imgs.shape[0], args.latent_dim))).cuda(args.gpu,
                                                                                                  non_blocking=True)
...
# Define the real and synthetic 3-dimensional time series data
def DTW_calculation(args, gen_net: nn.Module, test_data):
    from torch.utils import data
    real_data = data.DataLoader(test_data, batch_size=1, num_workers=1, shuffle=True)
    gen_net.eval()
    z = torch.cuda.FloatTensor(np.random.normal(0, 1, (len(real_data), args.latent_dim)))
    synth_data = gen_net(z).cpu().detach().numpy()
    synth_data = data.DataLoader(synth_data, batch_size=1, num_workers=1, shuffle=True)
    print(len(synth_data))

    real_seiz = []
    for i, (real_sig, label) in enumerate(real_data):
    ...
    syn_seiz = []
    for i, (syn_sig) in enumerate(synth_data):
        syn_sig = syn_sig.cpu().detach().numpy()
        sig = syn_sig.reshape(syn_sig.shape[1], syn_sig.shape[3])
        sig = np.transpose(sig, (1, 0))
        syn_seiz.append(sig)
    # syn_seiz = np.array(syn_seiz)
    print(syn_seiz[0].shape)
    real_time_series = real_seiz
    synthetic_time_series = syn_seiz
    ...
    for i in range(len(real_seiz)):
        ...
        for j in range(len(synthetic_time_series)):
            distance, path = fastdtw(real_time_series[i], synthetic_time_series[j], dist=euclidean_3d)
            if distance < best_similarity:
                best_similarity = distance
        dtw_distance.append(best_similarity)

    # print("DTW distance between real and synthetic time series:", dtw_distance)
    return dtw_distance, np.mean(dtw_distance)
...
def compute_cos_similarity(args, gen_net: nn.Module, test_data):
    from torch.utils import data
    real_data = data.DataLoader(test_data, batch_size=1, num_workers=1, shuffle=True)
    gen_net.eval()
    z = torch.cuda.FloatTensor(np.random.normal(0, 1, (len(real_data), args.latent_dim)))
    ...
    for i in syn_seiz:  # Iterate over the total number of syn sample(whole batch)
    ...
    return x_similarity, y_similarity, z_similarity, mean_similarity


def mse_calc(args, gen_net: nn.Module, test_data):
    from torch.utils import data
    real_data = data.DataLoader(test_data, batch_size=1, num_workers=1, shuffle=True)
    gen_net.eval()
    z = torch.cuda.FloatTensor(np.random.normal(0, 1, (len(real_data), args.latent_dim)))
    ...
    # synthetic_samples is a list of synthetic samples
    # real_samples is a list of real samples

    # Compare each synthetic sample to the real sample that it is most similar to, according to some metric of similarity
    max_realdata = []
    for seiz in real_seiz:
        max_seiz = np.max(seiz)
        max_realdata.append(max_seiz)
    avg_max = np.mean(max_realdata)
    total_max = np.max(max_realdata)

    best_mse = []
    for synthetic_sample in syn_seiz:
        best_match = None
        best_similarity = 1000
        for real_sample in real_seiz:
            ...
            if similarity < best_similarity:
                best_match = real_sample
                best_similarity = similarity
        best_mse.append(best_similarity)
    final_mse_epoch = np.mean(best_mse)
    return final_mse_epoch, avg_max, total_max
